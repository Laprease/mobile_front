<template>
	<div class="goods_detail">
		...
		<div class="detail_body">
			<refresh-list v-model:freshMap="data.freshMap" @refresh="onRefresh" :finishedText="''" @onLoad="onLoad">
				<div class="carousel_box">
					<carousel-card v-if="data?.carouselList?.length"
						v-model:carouselList="data.carouselList"></carousel-card>
				</div>
		...
		<!-- 分享弹窗 -->
		<van-share-sheet v-model:show="data.showShare" title="立即分享给好友" :options="data?.options" @select="onSelect" />
	</div>
</template>
...
const { useCommon, useHome,useShopCard } = $globalStore
...
//下拉刷新
const onRefresh = () => {
	data.freshMap.listFinished = false;
	data.freshMap.listLoading = true;
	data.freshMap.refreshLoad = true;
	Promise.all([getGoodsdetal(), recommendGoods()]).then(() => {
...
//获取商品信息
const getGoodsdetal = async (goodsId) => {
	let res = await useCommon.getGoodsdetal()
	if (res.code === 20000) {
...
//推荐
const recommendGoods = async (onLoad) => {
	
	let res = await useHome.recommendGoods()
	if (res?.code === 20000) {
