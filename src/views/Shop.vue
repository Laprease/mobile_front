			...
			<lodding-card :isLodding="isLodding">
				<van-pull-refresh v-model="refreshLoad" @refresh="onRefresh" class="refresh_box">
					<div class="refresh_content" v-if="data?.cartList.length">
						<div class="goods_box" v-for="(item, index) in data?.cartList" :key="index">
							<van-swipe-cell class="swipe_cell">
								<div class="card_box">
									<van-checkbox style="margin-left: 10px" v-model="item.checked"
										@change="oneCheckbox(item)"></van-checkbox>
									<van-card class="card_body" :min="1" :max="999" centered :desc="item?.desc"
										:title="item?.name" :thumb="item?.img" @click.stop="goto($event, item)">
										<template #price>
										...
										<template #footer>
											<van-stepper v-model="item.num" @plus="plus(item)" @minus="minus(item)"
												@blur="stpBlur(item)" @overlimit="stplimit" :min="1" :max="99" theme="round"
												button-size="22" />
										</template>
									</van-card>
								</div>
								...
								<template #right>
									<div style="height: 100%">
										<van-button style="height: 100%" square @click="delGoods(item)" type="danger"
											text="删除" />
									</div>
								</template>
							</van-swipe-cell>
...
// 操作购物车数量接口
const changeGoods = async (parmas) => {
	return await useShopCard.changeGoods(parmas);
};
